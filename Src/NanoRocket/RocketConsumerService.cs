                ...
                while (!_consumerCts.Token.IsCancellationRequested)
                {
                    try
                    {
                        _logger.LogInformation(
                            $"ConsumerGroup [{consumerGroup}] is connecting RocketMQ, Topic: [{topic}]...");
                    ...
                    {
                        _logger.LogError($"Failed to connect to RocketMQ: {e.Message}");
                        await Task.Delay(TimeSpan.FromMinutes(1), _consumerCts.Token);
                    }
                }
            }, _consumerCts.Token);
                        ...
                        if (_simpleConsumer != null)
                        {
                            ...
                            if (messageViews != null && messageViews.Any())
                            {
                                foreach (var message in messageViews)
                                {
                                    ...
                                    _logger.LogInformation(
                                        $"[{_configKey}] Message is acknowledged successfully, message-id={message.MessageId}");
